 ...
 public:
  ...
  void OnLoadingStateChange(CefRefPtr<CefBrowser> browser, bool loading, bool canBack, bool canForward) override {
    ...
    if (!loading && safeUrl(currentUrl.ToString()) == "https://oauth.yandex.ru/verification_code") {
      browser->GetMainFrame()->ExecuteJavaScript(R"JS((()=>{if(window.__o360OAuthCodeProbe)return;let attempts=0;window.__o360OAuthCodeProbe=setInterval(()=>{const elements=[...document.querySelectorAll('input,code,pre,[data-testid*=code]')];const values=elements.flatMap(el=>[el.value||'',el.innerText||el.textContent||'']).map(value=>value.trim());let code=values.find(value=>/^[A-Za-z0-9_-]{7,128}$/.test(value));if(!code){const text=document.body?.innerText||'';code=(text.match(/\b[A-Za-z0-9_-]{12,128}\b/g)||[]).find(value=>/\d/.test(value)&&/[A-Za-z]/.test(value))}if(code){clearInterval(window.__o360OAuthCodeProbe);console.log('__O360_OAUTH__'+JSON.stringify({code}))}else if(++attempts>80)clearInterval(window.__o360OAuthCodeProbe)},250)})())JS", currentUrl, 0);
    }
