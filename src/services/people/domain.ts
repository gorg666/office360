...
export function isValidPersonEmail(value: string): boolean {
  const normalized = normalizePersonEmail(value);
...
export function personIdentityFromEmail(
  ...
  const sources = uniqueSources(options.sources?.length ? options.sources : [source]);
  return {
    ...
    ...(clean(options.displayName) ? { displayName: clean(options.displayName) } : {}),
    ...(clean(options.firstName) ? { firstName: clean(options.firstName) } : {}),
    ...(clean(options.lastName) ? { lastName: clean(options.lastName) } : {}),
    ...(clean(options.jobTitle) ? { jobTitle: clean(options.jobTitle) } : {}),
    ...(clean(options.organization) ? { organization: clean(options.organization) } : {}),
    ...(clean(options.department) ? { department: clean(options.department) } : {}),
    ...(clean(options.avatarUrl) ? { avatarUrl: clean(options.avatarUrl) } : {}),
  };
}
