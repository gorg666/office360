...
Google заявляет remote Free/Busy для других участников. Generic CalDAV включает его только после RFC 6638 scheduling discovery; Yandex остаётся local-derived self / remote unsupported. Permissions и invitation delivery не заявлены. Reminder metadata поддерживается отдельно: Google — defaults/none/popup/email, CalDAV/Yandex — multiple DISPLAY writes и DISPLAY/EMAIL reads; desktop notification delivery остаётся отдельной функцией. Google sync-token и CalDAV delta state пока не durable; generic CalDAV/Yandex честно используют `range-refresh`. Calendar write success обновляет локальный cache через единый CAL-104 reconciliation path.
