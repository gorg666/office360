...
- Смена Яндекс-аккаунта больше не очищает cookies активного CEF request context; опасный `cef_clear_session` удалён, переход выполняется через Passport account chooser без разрушения browser session.
- Телемост использует отдельный постоянный CEF request context для каждого локального Яндекс-аккаунта. Cookies, cache и Passport-сессии больше не смешиваются; при переключении активируется профиль выбранного аккаунта, а остальные браузеры остаются скрытыми.
- Переключение профиля сначала скрывает прежнее native CEF-окно и не перезапускает Passport во время QR-подтверждения; это устраняет наложение двух браузеров и сброс незавершённого входа.
...
- Яндекс Мессенджер в первую очередь использует обновляемый OAuth-токен активного почтового аккаунта; сохранённый bot-token применяется только при отсутствии подключённого Яндекс OAuth-аккаунта.
