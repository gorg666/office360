...
### PEOPLE-001 participant and ACL picker baseline

PEOPLE-001 replaces direct Calendar attendee and ACL email fields with the shared provider-neutral `PeoplePicker`. Calendar authoring preserves selected display names through `ParticipantRef` and keeps Required/Optional roles independent; Scheduling Assistant receives the same identity. Calendar ACL remains capability-gated and forwards only the selected email principal through `CalendarAclService`. Existing Yandex/Google ACL write safety and owner protections are unchanged.

Directory search uses the official read-only Yandex 360 organization/users endpoints only when the account's current admin grant carries the required organization/user read scopes. Missing permission falls back to contacts, recent recipients, account-scoped Calendar participant history and valid manual email. No schema migration, cloud write or persistent directory cache is introduced. Canonical contract: `docs/people/PEOPLE_PICKER.md`.

Automated PEOPLE-001 acceptance: TypeScript PASS; targeted People/Mail/Calendar/ACL — 9 files / 43 tests PASS; full Vitest — 268 files / 2,575 tests PASS; four-zone Calendar TZ matrix — 15 files / 186 tests in each of UTC, Europe/Moscow, America/New_York and Australia/Lord_Howe; production build PASS (CalendarPage 162.65 kB raw / 47.23 kB gzip; main 2,165.13 kB raw / 642.57 kB gzip); `cargo check` PASS with the same two pre-existing unused-variable warnings in `src/lib.rs:378`; Rust was not changed.

Safe Tauri acceptance: Mail To showed local/recent results and the explicit directory-permission fallback, selected a rich identity chip, and exposed Cc/Bcc through the same picker; no Send was used. Calendar Month and the create form loaded existing Yandex data, participant search used the same fallback, and the form was cancelled without Save. ACL discovery remained capability-gated as unsupported and exposed no mutation control. The Composer performed its existing local draft autosave; no external Mail, event, RSVP or ACL cloud write occurred.

Graphify: `graphify update .` → 7,156 nodes / 18,508 edges / 434 communities. `graphify diagnose multigraph` PASS: 0 unverified code nodes, missing/dangling endpoints, self-loops or exact duplicate edges. Saved community labels remain stale; graph integrity is unaffected.
